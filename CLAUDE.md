...
| File | Role |
|------|------|
| `index.html` | National landing page: clickable US map (all 50 states + DC), link to states-list page, voter-turnout facts section, IP-geolocation home-state pulse |
| `states.html` | States in list format — static `<nav class="state-list">` of 51 real links (keyboard / screen-reader / crawler / slow-CDN route). Linked from under the map on `index.html` |
...
| `de.html` | Delaware — sixth fully built state (3 counties, 1 at-large seat). ✔ Sept 15, 2026 primary SETTLED (DOE open-data CSV, data_as_of Sept 16 00:59 ET; unofficial pending county cert ~Fri Sep 18). November: Senate **Coons (D) vs. Katz (R)**; House **McBride (D) vs. Arminio (R)**; AG **Jennings (D) vs. Graham (R)**; OPEN Treasurer **Lauzen (D) vs. Ramone (R)**; Auditor York (D) / Callaway (R) / Cassidy (I). No LOCAL_RACES yet |
...
| `ri.html` | Rhode Island — ninth fully built state (5 counties, 2 districts, Sept 9, 2026 primary **SETTLED and BOE-certified Sep 15** — WPRI/CNN/AP/Ocean State Media-called nominees; certification confirmed (What's Up Newp / News From The States)). Statewide: Senate **Reed (D) vs. McKay (R)** (+ ind. Bahry); Governor **Foulkes (D) vs. Guckian (R)** (+ inds. Block/Gotra/C.D. Reynolds) after Foulkes unseated McKee; open AG **Ahern (D) vs. Gordon (R)**; Lt Gov **Matos (D) vs. Loughlin (R)**; SoS Amore (D) unopposed; Treasurer Diossa (D) vs. Credle (R); RI-1 Amo/Keenan; RI-2 Magaziner vs. Mellor (Skoly defeated; Dickinson failed petition). Locals noted: Providence mayor Dem Morales over Smiley; Pawtucket mayor Dem Greenman over Grebien. Built July 20, 2026. No LOCAL_RACES yet |
| `nh.html` | New Hampshire — tenth fully built state (10 counties incl. split Hillsborough/Merrimack, 2 districts, Sept 8, 2026 primary **SETTLED** — official SoS Senate/House tallies via InDepthNH Sep 10; gov still AP-unofficial pending full SoS package). OPEN Senate: **Pappas (D) vs. Sununu (R)**, Lean D; Gov **Ayotte (R) vs. Warmington (D)**; OPEN NH-1: **Stefany Shaheen (D) vs. Anthony DiLorenzo (R)**, Likely D; NH-2: **Goodlander (D) vs. Lily Tang Williams (R)**, Likely D. Built July 20, 2026. No LOCAL_RACES yet |
...
| `la.html` | Louisiana — **forty-sixth fully built state** (64 PARISHES, 6 districts). Built Sept 3, 2026 from ms.html — the first page on the site to use `countyNoun: parish`. ⚠⚠ **NOV 3 IS ONLY THE FIRST ROUND FOR U.S. HOUSE, WITH A RUNOFF ON DEC 12** — Act 7 of the 2026 Regular Session put congressional races back into Louisiana's majority-vote open primary, so all parties share one ballot and a candidate wins outright only with a MAJORITY. **For U.S. Senate, Supreme Court, PSC and BESE, Nov 3 IS the general — and it is decided by PLURALITY with no runoff**, because those came through closed party primaries (Act 1 of the 2024 FIRST Extraordinary Session). **Never call Nov 3 the general election for a Louisiana House race.** ⚠ **BUILT ON ACT 2 OF 2026** (SB 121, signed May 29), the post-*Callais* map — validated three ways including a currency test against the SoS's live ballots in ALL 64 PARISHES with zero contradictions. **LA-06 was dismantled: D+8 → R+16, the largest shift in the state**, and **Cleo Fields is not running for Congress anywhere** — he is expected to seek his old state Senate seat, whose qualifying does not open until Oct 14. ⭐ **BILL CASSIDY LOST HIS OWN PRIMARY, finishing THIRD with 24.8%** — the first elected incumbent senator to lose renomination since Lugar in 2012; Julia Letlow (R) faces Jamie Davis (D), who carried all 64 parishes in his runoff. **LA-05 is also open** (Letlow vacated it). 39 certified House candidates read from the state's own list; ten constitutional amendments, all quoted verbatim. ⚠ PSC, BESE and the Supreme Court are DISTRICT races and are named rather than carded — and **all three Supreme Court seats were won unopposed and appear on no ballot**. No LOCAL_RACES yet |
| `state.html` | Generic per-state page, driven by URL param `?state=XX` (2-letter abbr). Renders that state's real county map + race data. NC/SC/GA/VA/MD/DE/NJ/NY/RI/NH/CT/VT/ME/MA/WV/OH/KY/IN/IA/IL/MS/AR/NE/NM/CO/OR/NV/SD/ID/MT/PA/TN/HI/AL/WI/MN/ND/KS/OK/AZ/FL/WY/MI/LA redirect to their dedicated pages |
...
- **State list (`states.html`):** a static `<nav class="state-list">` of 51 real links — the
  keyboard, screen-reader, crawler and slow-CDN route into the site. Linked from under the map on
  `index.html` ("States in list format"). Generated from BUILT by `tools/apply-index-fixes.js`;
  `tests/uniformity.js` fails if it disagrees with the pages on disk.
...
the state list on `states.html`) and `node tools/apply-state-page-fixes.js` (regenerates the ONE redirect list, in
...
- **⚠ STALE LINE (kept for history): once said six states left (AK CA LA MO TX) + DC — as of Sept 8 only CA/MO/TX remain unbuilt; AK and LA are built; DC is PARTIAL.** Louisiana is fully
...
Item 4 (voices) reads **0 missing / 1,553 candidates across 46 pages** at the end of the Sept 3 run — Michigan shipped
with 130 candidates and Louisiana with 41, both 0 gaps. The landing page was reviewed end-to-end and its dependent facts reconcile: the legend
needed only its count (44 → **46 states**), the footer already read September 2026, PARTIAL is still **{DC} alone**, and
the tier arithmetic checks out — 46 built + 1 marquee + 4 starter = 51. The prose has been fully state-agnostic since
...
The [Verify] total went 3,515 → **3,981**: Michigan's 394 and Louisiana's 89 honest markers, less 20 cleared by the
Kansas certification and a handful cleared elsewhere. ✅ **The time-sensitive calendar is still EMPTY.**
...
✅ **LOUISIANA WAS ALSO BUILT THIS RUN — the 46th state.** `tools/banked/la-build/` holds the derivation. Its map is Act 2 of the 2026
